...
publish_dir=$(mktemp -d /private/tmp/binary-gv-pages.XXXXXX)
trap 'rm -rf "$publish_dir"' EXIT HUP INT TERM

cd "$dashboard_root"
env PATH="$runtime_bin" pnpm run build:pages

git clone --depth 1 https://github.com/kz99/binary-gv-concrete-observatory.git "$publish_dir"
find "$publish_dir" -mindepth 1 -maxdepth 1 ! -name .git -exec rm -rf {} +
cp -R "$dashboard_root/dist/." "$publish_dir/"
...
git commit -m "Publish binary GV observatory"
